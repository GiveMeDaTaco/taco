...
class TemplateConditions(BaseModel):
    ...
    # BA filters are mandatory for *main* conditions but **optional** for
    # individual *channels*.  Using a default empty list allows omission in
    # the YAML while keeping the attribute always iterable downstream.
...
class ConditionsConfig(BaseModel):
    ...
    def assign_names_and_validate(cls, self) -> 'ConditionsConfig':  # type: ignore[name-defined]
        # Ensure *main* channel includes at least one BA filter – remains a
        # hard requirement for the pipeline logic.
        if not self.main.BA:
            raise ValueError("'eligibility.conditions.main.BA' must contain at least one check")
...
class PreSQLFile(BaseModel):
    ...
    # When False, errors executing this file's SQL/analytics are *ignored* (logged as warnings)
    # and the pipeline continues.  Default True keeps legacy "fail-fast" behaviour.
    error: bool = True
    ...
    def _coerce_legacy(cls, v):  # type: ignore[name-defined]
        ...
        if isinstance(v, str):
            # Legacy string form => treat as mapping with default settings
            return {"path": v}
...
class AppConfig(BaseModel):
    ...
    # Optional list of post-sql file definitions executed *after* output stage
    post_sql: Optional[List[PreSQLFile]] = None
    # Optional text file containing newline-delimited DB user IDs granted access
    user_access_list_file: Optional[str] = None
    ...
    # Re-use the same path validation for *post_sql*
    @field_validator('post_sql', mode='after')
    def _validate_post_sql_paths(cls, v):  # type: ignore[name-defined]
        if not v:
            return v
        import os
        missing = [p.path for p in v if not os.path.isfile(p.path)]
        if missing:
            raise ValueError(f"post_sql files not found: {missing}")
        return v

    @field_validator('user_access_list_file', mode='after')
    def _validate_user_list_path(cls, v):  # type: ignore[name-defined]
        if v is None:
            return v
        import os
        if not os.path.isfile(v):
            raise ValueError(f"user_access_list_file not found: {v}")
        return v
